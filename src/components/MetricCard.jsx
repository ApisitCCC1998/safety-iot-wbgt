...
export default function MetricCard({ label, value, unit, icon, variant, wbgtValue }) {
  const isWbgt = variant === 'wbgt'
  const tier = isWbgt ? getWBGTTier(wbgtValue ?? 0) : null
  const colors = isWbgt ? getTierColors(tier.tier) : null
  const accentText = isWbgt ? colors.text : variantStyles[variant]?.accent ?? 'text-slate-600'
  const iconBg = isWbgt ? '' : variantStyles[variant]?.iconBg ?? ''
